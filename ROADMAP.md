...
- Repo restructured as self-hosted marketplace (`second-brain-projects`): plugin moved to `plugins/logseq-brain/`, `marketplace.json` added at repo root. Install via `/plugin marketplace add vlad-aleksandrov/LogseqBrain` + `/plugin install logseq-brain@second-brain-projects`.
