...
The plugin is distributed as a self-hosted marketplace from `vlad-aleksandrov/LogseqBrain`. The `.plugin` archive is a **build artifact** — gitignored (`*.plugin`), not checked in. Edit files under `skills/` and `.claude-plugin/`, never inside an archive.
...
- Current version is in `plugins/logseq-brain/.claude-plugin/plugin.json` AND `.claude-plugin/marketplace.json` — bump both on release. `ROADMAP.md` lists shipped/current/future phases — verify shipped status by reading the skills, not the roadmap.
